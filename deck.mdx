...
<CodeSurferColumns themes={[github, vsDark]} sizes={[1,2]}>
...
JavaScript Library for building state machines declaratively.

It supports the basics of state machines (states, transitions, actions, final states) among other things.

XState allows to define state machines using JSON syntax.
...
Each state can define its allowed transitions, and other handy features, such as: whether the state is final, entry and exit transitions, actions (a.k.a: side effects) for each state, etc.
...
XState also supports extended state (or context) to save additional information in the state machine, related to the states.

You can use state actions to mutate this extended state.
...
From the static definition, we can determine the next state of the state machine with the **PURE** state-transition function.

First, let's get the definition of the current state, and determine the target state for the provided event.
...
function createMachine(machineDef) {
  const machine = {
    ...
    transition(state, event) {
      ...
      // TODO: execute entry and exit actions of each state
...
Finally, we can get the definition of the next state, execute other actions – such as modifying context, entry and exit transitions –, and return the next state.
...
function createMachine(machineDef) {
  const machine = {
    ...
    transition(state, event) {
      ...
      // Modify machine state and context
...
If you noticed, that is just a reducer function! We can use it in `Array.prototype.reduce` to resolve a sequence of events
...
const finalState = statesSequence.reduce(
